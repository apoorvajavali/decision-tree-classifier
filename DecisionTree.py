...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.label = max(self.predictions.items(), key=operator.itemgetter(1))[0]
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print(spacing + "Predicted Class:", node.label)
...
def getLeafNodes(node, leafNodes=[]):
    ...
    if isinstance(node, Leaf):
        return [node]
    leafNodes = getLeafNodes(node.true_branch) + getLeafNodes(node.false_branch)
...
def getInnerNodes(node, innerNodes=[]):
    ...
    if isinstance(node, Leaf):
        return []
    innerNodes = getInnerNodes(node.false_branch) + getInnerNodes(node.true_branch)
...
def getInnerNodesId(t, idList = []):
    # Returns a list of ids of nodes that are one level above the leaf nodes
    if isinstance(t, Leaf):
        return
    getInnerNodesId(t.false_branch)
    getInnerNodesId(t.true_branch)
    if isinstance(t.false_branch, Leaf) or isinstance(t.true_branch, Leaf):
        idList.append(t.id)
    return idList
